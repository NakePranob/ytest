...
const validate = (state: any): FormError[] => {
    ...
    if (!state.password) errors.push({ path: 'password', message: 'password-policy-required' })
    if (!state.c_password) errors.push({ path: 'c_password', message: 'confirm-password-policy-required' })
...
<template>
    <div class="max-w-[420px] w-full flex flex-col items-center justify-center">
        ...
        <div class="flex flex-col justify-center gap-2 mt-10 mb-8 w-full">
            <h1 class="text-[32px] font-bold text-primary-app dark:text-primary-app-400">{{ $t('change-password-title')
                }}</h1>
            <p class="text-base">
        ...
        <UForm :validate="validate" :state="state" class="space-y-4 w-full" @submit="onSubmit">
            ...
            <div>
                <UButton :leading="isLoading" type="submit" block size="xl" :padded="false" :ui="{
                    font: '!text-base',
